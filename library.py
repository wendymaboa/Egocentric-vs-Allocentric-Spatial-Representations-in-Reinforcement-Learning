...
def tokenize(text, vocab, max_length=20):
    """
    Tokenize text using vocabulary, automatically handling unknown tokens
    
    Args:
        text: string to tokenize
        vocab: dictionary mapping tokens to indices (will be modified if needed)
    ...
    # Convert tokens to indices, adding unknown tokens to vocab
    var_indexed_text = []
    for token in tokens:
        if token in vocab:
            var_indexed_text.append(vocab[token])
        else:
            # Add new token to vocabulary
            new_idx = len(vocab)
            vocab[token] = new_idx
            var_indexed_text.append(new_idx)
            # print(f"Added new token to vocab: '{token}' -> {new_idx}")
    
    # Pad or truncate to max_length
    if len(var_indexed_text) < max_length:
        # Pad with <PAD> token (index 0)
        var_indexed_text.extend([0] * (max_length - len(var_indexed_text)))
    else:
        # Truncate
        var_indexed_text = var_indexed_text[:max_length]
    
    return np.array(var_indexed_text)
